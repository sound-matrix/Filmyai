...
export default async function FilmDetailPage({ params }: Props) {
  ...
  return (
    <article>
      <div className="mb-6 flex aspect-video items-center justify-center rounded-xl bg-filmy-elevated text-filmy-muted">
      ...
      <div className="flex flex-col gap-6 md:flex-row">
        ...
        <div className="flex-1">
          ...
          <h1 className="mb-3 font-display text-2xl font-bold sm:text-3xl">{film.title}</h1>
          <p className="mb-4 text-filmy-muted">{film.synopsis}</p>
          <p className="mb-6 text-sm">
            Access: <span className="font-medium text-filmy-fg">{film.access_rule}</span>
          </p>
          <Link
